...
Interval Overlap Arithmetic: Automated detection of overlapping time slots using the java.time API (
S
t
a
r
...
Navigate to Resource Management (Option 1) 
→
 View All Resources (Option 2).
...
Navigate to Booking Management (Option 3) 
→
 Create New Booking (Option 1).
Try booking resource CR-101 for 2026-10-15 09:30 to 2026-10-15 11:30.
The system will throw and catch BookingConflictException, gracefully informing the user: [BOOKING REJECTED] Conflict detected with Booking BK-1001: 2026-10-15 09:00 to 2026-10-15 12:00.
3. Demonstrate Maintenance Status Restriction
Try to book resource PRJ-01 (which is under maintenance for a burnt lamp).
The system rejects the reservation with ResourceUnderMaintenanceException: [BOOKING REJECTED] Resource PRJ-01 is currently UNDER MAINTENANCE.
4. Demonstrate Dynamic Maintenance Resolution
Navigate to Maintenance Management (Option 4) 
→
 Update Maintenance Status (Option 3).
...
Navigate to Reports & Analytics (Option 5) 
→
 Export Report to File (Option 4).
...
Terminal Non-Crash Guarantee: Typing alphabetic characters into integer/date prompts triggers friendly validation messages rather than unhandled InputMismatchException or NumberFormatException.
Time Paradox Prevention: Booking or event start times must precede end times (
S
t
a
r
...
No Duplicate Resource IDs: Attempting to add an existing Resource ID throws DuplicateRecordException.
Duplicate Attendee Guard: Prevents the same student email from being registered twice for a single event.
Cascading Integrity: Cancelling an event cascades and marks associated resource bookings as cancelled.
Graceful Stream EOF: Exiting or piping inputs terminates cleanly without NoSuchElementException.
